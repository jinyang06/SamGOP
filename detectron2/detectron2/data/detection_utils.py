...
def generate_attention_map(image_, heatmap):
    ...
    amap = cv2.cvtColor(hmap, cv2.COLOR_RGB2BGR)
    ...
    normed_mask = cv2.addWeighted(img_new, 0.6, normed_mask, 0.5, 0)
    imgs=cv2.cvtColor(normed_mask, cv2.COLOR_BGR2RGB)
    imgs=Image.fromarray(imgs)
    # img=Image.open(res)
    return imgs
    # img.show()

def generate_attention_map_(image_, heatmap):
    unloader = transforms.ToPILImage()
    a = heatmap
    hmap = a.cpu().clone()
    hmap = unloader(hmap)
    ...
    normed_mask = cv2.applyColorMap(normed_mask, cv2.COLORMAP_JET)
    # normed_mask = cv2.applyColorMap(normed_mask, cv2.COLORMAP_BONE)
    normed_mask = cv2.addWeighted(img_new, 0.3, normed_mask, 0.7, 0)
    imgs=cv2.cvtColor(normed_mask, cv2.COLOR_BGR2RGB)
    imgs=Image.fromarray(imgs)
    # img=Image.open(res)
    return imgs
...
def transform_keypoint_annotations(keypoints, transforms, image_size, keypoint_hflip_indices=None):
    ...
    """
    # (N*3,) -> (N, 3)
    keypoints = np.asarray(keypoints, dtype="float64").reshape(-1, 2)
    ...
    if do_hflip:
        ...
        keypoints_xy[:, 0] = 1024 - keypoints_xy[:, 0]
        keypoints_xy[:, 1] = 1024 - keypoints_xy[:, 1]
...
def annotations_to_instances_goo(annos, image_size, mask_format="polygon"):
    """
    Create an :class:`Instances` object used by the models,
    from instance annotations in the dataset dict.
