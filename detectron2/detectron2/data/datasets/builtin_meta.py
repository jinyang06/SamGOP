...
COCO_CATEGORIES = []
id = 1
for index, (item, cate) in enumerate(zip(COCO_CATEGORIES_, names)):
    item['name'] = cate
    item['id'] = id
    id += 1
    COCO_CATEGORIES.append(item)
x=0
...
#     {"color": [174, 57, 255], "isthing": 1, "id": 25, "name": "bear"},
# ]
# with open('/data1/jinyang/TransGOP/MaskDINO2/datasets/coco/annotations/cate.txt', 'r') as f:
#     names = f.readlines()
# list_ = []
# for name1 in names:
#     name_ = name1.replace('\n', '')
...
def _get_coco_instances_meta():
    ...
    assert len(thing_ids) == 24, len(thing_ids)
