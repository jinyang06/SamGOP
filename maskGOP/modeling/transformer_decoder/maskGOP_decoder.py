...
from .dino_decoder import TransformerDecoder, DeformableTransformerDecoderLayer, DeformableTransformerDecoderLayerForHead
...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        decoder_layer_head = DeformableTransformerDecoderLayerForHead()
        self.decoder_head = DeformableTransformerDecoder(decoder_layer_head, 3, True)
        self.reference_points_head = nn.Linear(hidden_dim, 2)
        self.query_embed_head = nn.Embedding(num_queries, hidden_dim * 2)
        ...
        # self.head_decoder = Transformer_decoder(d_model=256, nhead=8,
        #          num_decoder_layers=6, dim_feedforward=2048, dropout=0.0,
        #          activation="relu", normalize_before=False,
        #          return_intermediate_dec=False)
        # self.head_query_embed = nn.Embedding(100, 256)


        self.hidden_dim = hidden_dim
        ##########################################################
        self.head_box_embed_ = MLP(hidden_dim, hidden_dim, 4, 3)
        self.head_class_embed_ = nn.Linear(hidden_dim, 2)
        head_box_embed_layerlist = [self.head_box_embed_ for i in range(3)]
        head_class_embed_layerlist = [self.head_class_embed_ for i in range(3)]
        self.head_bbox_embed = nn.ModuleList(head_box_embed_layerlist)
        self.head_class_embed = nn.ModuleList(head_class_embed_layerlist)
        ##########################################################
        ...
        self.ref_point_head = MLP(query_dim // 2 * 256, 256, 256, 2)
    ...
    def forward(self, x, mask_features, object_pos_, object_mem_, object_mask_,masks,targets=None):
        """
        ...
        predictions_class = []
        predictions_mask = []
        predictions_mask_embed = []
        if self.two_stage:
            output_memory, output_proposals = gen_encoder_output_proposals(src_flatten, mask_flatten, spatial_shapes)
            output_memory = self.enc_output_norm(self.enc_output(output_memory))
            # output_cls_head = self.head_class_embed(output_memory)
            # output_box_head = self.head_box_embed(output_memory)
            # topk_head_proposals = torch.topk(output_cls_head.max(-1)[0], 300, dim=1)[1]
            # output_head_box = torch.gather(output_box_head, 1,
            #                                        topk_head_proposals.unsqueeze(-1).repeat(1, 1, 4))  # unsigmoid
            # tgt_head_undetach = torch.gather(output_memory, 1,
            #                             topk_head_proposals.unsqueeze(-1).repeat(1, 1, self.hidden_dim))  # unsigmoid
            # tgt_head_undetach = tgt_head_undetach.transpose(0, 1)
            # decoder_head_output = self.decoder_norm(tgt_head_undetach)
            # decoder_head_output = decoder_head_output.transpose(0, 1)
            # outputs_head_class = self.head_class_embed(decoder_head_output)
            # x=0
            ...
            outputs_class, outputs_mask, outputs_mask_embed = self.forward_prediction_heads(tgt_undetach.transpose(0, 1), mask_features)
        ...
        if self.initial_pred:
            outputs_class, outputs_mask, outputs_mask_embed = self.forward_prediction_heads(tgt.transpose(0, 1), mask_features, self.training)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            # predictions_mask_embed.append(outputs_mask_embed)
        if self.dn != "no" and self.training and mask_dict is not None:
            refpoint_embed=torch.cat([input_query_bbox, refpoint_embed],dim=1)
        # tgt_to_gaze = tgt.clone()
        # refpoint_embed_gaze = refpoint_embed.clone()
        # refpoint_embed_gaze = refpoint_embed
        ...
        query_embed_head = self.query_embed_head.weight
        bs_head, _, c_head = object_mem_.shape
        query_embed_head, tgt_head = torch.split(query_embed_head, c_head, dim=1)
        query_embed_head = query_embed_head.unsqueeze(0).expand(bs, -1, -1)
        tgt_head = tgt_head.unsqueeze(0).expand(bs_head, -1, -1)
        reference_points_head = self.reference_points_head(query_embed_head).sigmoid()
        init_reference_out_head = reference_points_head
        hs_head, inter_references_head = self.decoder_head(tgt_head, reference_points_head, object_mem_,spatial_shapes,level_start_index,valid_ratios,query_embed_head, object_mask_)

        #####################################################################################################
        # Head Detection
        #####################################################################################################
        # head_mask = object_mask
        # hs_head = self.head_decoder(object_mem, head_mask, self.head_query_embed.weight, object_pos.flatten(2).permute(0,2,1))

        outputs_classes_head = []
        outputs_coords_head = []
        for lvl_head in range(hs_head.shape[0]):
            if lvl_head == 0:
                reference_head = init_reference_out_head
            else:
                reference_head = inter_references_head[lvl_head - 1]
            reference_head = inverse_sigmoid(reference_head)
            outputs_class_head = self.head_class_embed[lvl_head](hs_head[lvl_head])
            tmp = self.head_bbox_embed[lvl_head](hs_head[lvl_head])
            if reference_head.shape[-1] == 4:
                tmp += reference_head
            else:
                assert reference_head.shape[-1] == 2
                tmp[..., :2] += reference_head
            outputs_coord_head = tmp.sigmoid()
            outputs_classes_head.append(outputs_class_head)
            outputs_coords_head.append(outputs_coord_head)
        outputs_class_head = torch.stack(outputs_classes_head)
        outputs_coord_head = torch.stack(outputs_coords_head)


        # output_cls_head = self.head_class_embed(hs_head)
        # output_box_head = self.head_box_embed(hs_head)
        interm_head_outputs = dict()
        interm_head_outputs['pred_head_logits'] = outputs_class_head[-1]
        interm_head_outputs['pred_head_boxes'] = outputs_coord_head[-1]
        interm_head_outputs['aux_outputs_head'] = self._set_aux_loss_head(outputs_class_head, outputs_coord_head)
        ######################################################################################################

        # tgt_to_gaze = hs[-1].clone().detach()
        refpoint_embed_gaze = references[-1].clone()
        for i, output in enumerate(hs):
            outputs_class, outputs_mask, outputs_maskd_embed = self.forward_prediction_heads(output.transpose(0, 1), mask_features, self.training or (i == len(hs)-1))
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_mask_embed.append(outputs_maskd_embed)

        # predictions_mask_embed_for_gaze = predictions_mask_embed[-1].clone().detach()
        tgt_to_gaze = predictions_mask_embed[-1].clone().detach()
        ...
        reference_points = refpoint_embed_gaze.transpose(0, 1).to(device)
        ...
        # object_mem = tgt_to_gaze
        object_mem = tgt_to_gaze
        object_pos = query_pos.permute(1, 0, 2)
        object_mask = torch.zeros((tgt_to_gaze.size(0), tgt_to_gaze.size(1)), device=device, dtype=torch.bool)

        if self.two_stage:
            out['interm_outputs'] = interm_outputs
            out['interm_head_outputs'] = interm_head_outputs

        return out, mask_dict, object_mem, object_pos, object_mask, interm_head_outputs
    ...
    def _set_aux_loss_head(self, outputs_class, outputs_coord):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [{'pred_head_logits': a, 'pred_head_boxes': b}
...
#
#
#         decoder_layer = TransformerDecoderLayer_(d_model, nhead, dim_feedforward,
#                                                 dropout, activation, normalize_before)
#         decoder_norm = nn.LayerNorm(d_model)
#         self.decoder = TransformerDecoder_(decoder_layer, num_decoder_layers, decoder_norm,
#                                           return_intermediate=return_intermediate_dec)
#
...
#         # query_embed = query_embed.to(torch.float16)
#         # try:
