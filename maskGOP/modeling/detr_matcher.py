...
from maskGOP.utils.box_ops import generalized_box_iou, box_cxcywh_to_xyxy
...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def memory_efficient_forward(self, outputs, targets, cost=["cls", "box"]):
        """More memory-friendly matching. Change cost to compute only certain loss in matching"""
        bs, num_queries = outputs["pred_head_logits"].shape[:2]

        indices = []

        # Iterate through batch size
        for b in range(bs):
            out_bbox = outputs["pred_head_boxes"][b]
            if 'box' in cost:
            ...
            out_prob = outputs["pred_head_logits"][b].sigmoid()  # [num_queries, num_classes]
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, cost=["cls", "box"]):
