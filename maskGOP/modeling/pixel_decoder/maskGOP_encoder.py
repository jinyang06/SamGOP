...
class MSDeformAttnTransformerEncoderOnly(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds):
        ...
        lvl_pos_embed_ = lvl_pos_embed_flatten.detach().clone()
        mask_flatten_ = mask_flatten.detach().clone()
        return memory, spatial_shapes, level_start_index, lvl_pos_embed_, mask_flatten_
...
class MaskDINOEncoder(nn.Module):
    ...
    def forward_features(self, features, masks):
        ...
        if self.feature_order != 'low2high':
            srcs = srcsl
            pos = posl
        y, spatial_shapes, level_start_index, pos_faltten, mask_flatten = self.transformer(srcs, masks, pos)
        object_mem = y
        object_mask_flatten = mask_flatten
        object_pos = pos_faltten
        bs = y.shape[0]
        ...
        return self.mask_features(out[-1]), out[0], multi_scale_features, object_pos, object_mem, object_mask_flatten
